...
document.querySelector("#CreateServer_CreateButton").onclick = () => {
    ...
    if (/\/|\\|:|\*|\?|<|>|\|/g.test(serverName)) {
        confirm(
            "A server name can't contain any of the following characters: / \\ : * ? < > |",
            ["Ok"],
            (ans) => { },
        );
        return;
    ...
    confirm(
        `Do you really want to create the server "${serverName}" with these settings? The settings can be changed afterwards.`,
        ["Yes", "Cancel"],
        (ans) => {
            if (ans === 0) {

...
const clickProps = () => {
    if (!fs.existsSync(path.join(DIR.SERVER, "./server.properties"))) {
        dialog.showErrorBox("The properties file could not be found.", "Make sure you ran the server at least once before trying to change the properties.");
...
document.querySelector("#Properties_Save").onclick = () => {
    checkOpen(result => {
        if (!result) {
            ...
            dialog.showErrorBox("You cannot save new properties while the server is running!", "");
        }
    });
};
    ...
    checkOpen(result => {
        if (!result) {
            dialog.showMessageBox(null, {
                ...
                title: "Minecraft Server Shell",
                buttons: ["Yes", "Cancel"],
                message: "Are you sure you want to reset your server properties?",
            }).then(response => {
                if (response.response === 0) {
                    setLoad(true, "Saving Changes");
                    if (!fs.existsSync(path.join(DIR.SERVER, "default_server.properties"))) {
                        dialog.showErrorBox("The default properties file is missing.", "");
                    } else {
            ...
            dialog.showErrorBox("You cannot reset the properties while the server is running!", "");
        }
    });
};